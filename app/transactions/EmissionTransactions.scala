...
/**
 * Where emission transactions get their wallet funds. Backed by [[WalletManager]] in production, so
 * that this and the rollup transactions cannot select the same UTXO — the loser of that race sees a
 * double spend indistinguishable from anyone else's.
 */
trait WalletSource {

  /** Reserve inputs covering `value` nanoERG and, if given, `token`. Throws if the wallet is short. */
  def take(value: Long, token: Option[Token]): Seq[InputUTXO]

  /** Hand change outputs back so the next transaction in a chained run can spend them. */
  def giveBack(boxes: Seq[InputUTXO]): Unit

  /** Un-reserve inputs that were taken but never spent. */
  def release(boxes: Seq[InputUTXO]): Unit
}

/** One built emission transaction and which path produced it. */
case class EmissionSpend(tx: SignedTransaction, kind: String)
 ...
 *
 * Structured as [[RollupTransactions]] is: pure builders taking the boxes they need and returning a
 * signed transaction, with [[EmissionHandler]] owning the timers and the broadcasting.
 *
...
class EmissionTransactions(prover: NodeWallet,
  ...
  def emissionTip(ctx: BlockchainContext): EmissionTip = {
    ...
    val confirmed = nodeApi
      .unspentBoxesByTokenId(nft, Paging.all(8), SortDirection.Desc, MempoolOptions.ConfirmedOnly)
  ...
  def buildQueueSpends(ctx: BlockchainContext,
    ...
    try {
          ...
          case Some(queueIn) =>
            Try(lenderEntry(queueIn.registers(1).getValue.asInstanceOf[SigmaProp])).toOption match {
              ...
              case Some(newEntry) if st.holds(newEntry) =>
                ...
                built :+= EmissionSpend(sTx, EmissionSpend.Clear)
                em = chainOn(sTx, funding)
              ...
              case Some(_) =>
                ...
                if (!st.bootstrapping && retiring.isEmpty) {
                  ...
                  built :+= EmissionSpend(sTx, EmissionSpend.Activate)
                  em = chainOn(sTx, funding)
                }
            }
        }
      ...
      funding.finish(keepReservations = funded && completed, returnChange = false)
    ...
    if (room <= 0) {
      ...
      if (keys.isEmpty) {
        ...
        try while (!stop && remaining.nonEmpty) {
          ...
          funding.finish(keepReservations = sent.nonEmpty, returnChange = sent.nonEmpty)
        sent
      }
    }
  }
   ...
   */
  class FundingSource {
    private var chained: Vector[InputUTXO] = Vector.empty
    private var reserved: Vector[InputUTXO] = Vector.empty
    private var used = Set.empty[String]

    /** Chain a change output back in, so the next transaction in a run can spend it. */
    def push(box: InputUTXO): Unit = chained = box +: chained

    def take(value: Long, token: Option[Token] = None): Seq[InputUTXO] = {
      val fromChain = drawFrom(chained.filterNot(b => used.contains(b.id.toString)), value, token)
      ...
      val fromWallet =
        if (ergShort == 0 && tokenShort.isEmpty) Seq.empty[InputUTXO]
          ...
          val drawn = walletSource.take(ergShort, tokenShort)
          reserved ++= drawn
          drawn
        }
     ...
     */
    def finish(keepReservations: Boolean, returnChange: Boolean): Unit = {
      if (!keepReservations && reserved.nonEmpty) walletSource.release(reserved)
      if (returnChange) {
        val leftover = chained.filterNot(b => used.contains(b.id.toString))
  ...
  private def chainOn(sTx: SignedTransaction, funding: FundingSource): InputUTXO = {
    val outputs = sTx.getOutputsToSpend.asScala.toSeq.map(InputUTXO(_))
    outputs.drop(1)
      .find(_.contract.ergoTreeHex == walletTree)
      .foreach(funding.push)
    outputs.head
  }
