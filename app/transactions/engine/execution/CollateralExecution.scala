 ...
 */
class CollateralExecution(nodeContext: NodeContext,
                                        config: Configuration,
                                        system: ActorSystem,
                                        walletManager: ActorRef, alive: () => Boolean = () => true)
  ...
  private lazy val txs: EmissionTransactions =
    new EmissionTransactions(nodeContext.getNodeWallet, executionNode,
  ...
  def invalidateSnapshots(): Unit = synchronized {
    viewGeneration += 1L
    cachedView = None
    cachedMarket = None
  }
    ...
    reply match {
      ...
      case RewardsClaimed(chunks) =>
        CollateralRewardClaimResult(
          claims = chunks.map(c => CollateralRewardClaimedChunk(c.txId, c.boxes, c.nanoErgs.toString, c.outcome)),
          claimedNanoErgs = chunks.filter(_.outcome == "accepted").map(c => BigInt(c.nanoErgs)).sum.toString)
