 ...
 * @param discoverPools also broadcast for every other deployment found at the pool contract's template,
 *                  served only once its contracts, compiled from its own ids, reproduce its pool and vault
 *                  trees exactly. Block candidates keep executing the canonical pool alone.
 */
case class LithosDexBatchingConfig(batching: BatchingConfig, autoFlush: Boolean, discoverPools: Boolean = false)
