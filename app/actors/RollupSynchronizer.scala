...
class RollupSynchronizer @Inject()(config: Configuration, @Assisted rollupBlockId: String, @Assisted ctx: BlockchainContext, @Assisted prover: NodeWallet, cacheApi: SyncCacheApi) extends Actor {
      ...
      case LFSMPhase.HOLDING =>
        transform.output.ergoTree match {
          case hContract if hContract == relErgoTrees.head =>
            submissionTransform(tree, transform)
          case eContract if eContract == relErgoTrees(1) =>
            holdingTransform(tree, transform)
        }
      case LFSMPhase.EVAL =>
        transform.output.ergoTree match {
          case eContract if eContract == relErgoTrees(1) =>
            fraudProofTransform(tree, transform)
          case pContract if pContract == relErgoTrees(2) =>
            evalTransform(tree, transform)
        }
      case LFSMPhase.PAYOUT =>
        transform.output.ergoTree match {
          case pContract if pContract == relErgoTrees(2) =>
            payoutTransform(tree, transform)
          case fullyPaid if !relErgoTrees.contains(fullyPaid) =>
            payoutTransform(tree, transform)
        }
    }
  }

  private def submissionTransform(tree: NISPTree, transform: Transform): NISPTree = {
  ...
  private def holdingTransform(tree: NISPTree, transform: Transform): NISPTree = {
    ...
    if (tree.hasMiner) {
      ...
      stopSync("No local miner in holding transform", tree)
    }
  }

  private def fraudProofTransform(tree: NISPTree, transform: Transform): NISPTree = {
    ...
    if (tree.hasMiner) {
      ...
      stopSync("No local miner in fraud proof transform", tree)
    }
  }
  ...
  private def evalTransform(tree: NISPTree, transform: Transform): NISPTree = {
    if (tree.hasMiner) {
      ...
      stopSync("No local miner in evaluation transform", tree)
    }
  }

  private def payoutTransform(tree: NISPTree, transform: Transform): NISPTree = {
    if (tree.hasMiner) {
      val dict = tree.dictionary
      ...
      if (paidMiner) {
        ...
        if (transform.output.ergoTree == Helpers.payoutContract(ctx).ergoTreeHex) {
          ...
          stopSync("No payout contract output in transform", tree)
        }
      }
    } else {
      stopSync("No local miner in payout transform", tree)
    }
  }
