...
import state.messages.SyncMessages.{CompletedInitSync, HandledBlock}
...
class PDSynchronizer @Inject()(cacheApi: SyncCacheApi) extends Actor with InjectedActorSupport {
  val logger: Logger = LoggerFactory.getLogger("PDSynchronizer")
...
//          }
  ...
  private def depositTransform(ls: LiquidityState, transform: DictionaryTransform, ctx: BlockchainContext) = {
    ...
    val deltaLPSupply = (PDHelpers.MAX_LIQ - nextLiq) - (PDHelpers.MAX_LIQ - ls.liquidity)
  ...
  private def swapTransform(ls: LiquidityState, transform: DictionaryTransform) = {
  ...
  private def withdrawTransform(ls: LiquidityState, transform: DictionaryTransform) = {
