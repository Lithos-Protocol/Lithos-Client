...
object StateFrame {
  ...
  private sealed trait OwnerCompletion {
    def incarnation: String
  }
  private final case class PollFinished(incarnation: String, result: Try[PollResult])
    extends OwnerCompletion
  private final case class CommitFinished(incarnation: String,
                                          block: BlockInfo,
                                          tipHeight: Int,
  ...
  private final case class AncestorLocated(incarnation: String,
                                           result: Try[Option[SyncCursor]],
                                           heights: NodeHeights) extends OwnerCompletion
  private final case class RollbackFinished(incarnation: String,
                                            result: Try[Any],
  ...
  private final case class DictionaryRepairPublished(incarnation: String, result: Try[Any])
  /** Identity and optional frozen catch-up tip for exactly one quarantine-repair cycle. */
  private final case class RepairCycle(incarnation: String, id: Long, maintenanceTip: Option[Int])
...
/**
 * Manages the flow of canonical blocks to the SyncHandler, which
 * owns the current committed state. Waits for commitments to complete
...
class StateFrame @Inject()(config: Configuration,
  ...
  private val handlers: Receive = {
    // Futures created by an older actor instance still target this ActorRef. They must not mutate the
    // replacement instance after an Akka restart.
    case completion: OwnerCompletion if completion.incarnation != incarnation => ()
    ...
    case PollFinished(_, Success(BatchFetched(blocks, tip))) =>
    ...
    case PollFinished(_, Success(AtTip(_))) =>
      inFlight = false
      cursor match {
    ...
    case PollFinished(_, Success(IndexBehind(at, heights))) =>
      inFlight = false
    ...
    case PollFinished(_, Success(ForkDetected(at, detail, heights))) =>
      ...
      (syncHandler ? GetRetainedCursors)
        .map {
          case RetainedCursors(values) => CursorsLoaded(incarnation, Success(values), heights)
          case other => CursorsLoaded(incarnation, Failure(new IllegalStateException(
            s"Unexpected retained-cursor response $other")), heights)
        }
        .recover { case ex => CursorsLoaded(incarnation, Failure(ex), heights) }
        .pipeTo(self)

    case PollFinished(_, Failure(ex)) => failPoll("Canonical block poll failed", ex)

    case CommitFinished(_, block, tip, Success(committedBlock: BlockCommitted)) =>
    ...
    case CommitFinished(_, _, _, Success(SyncUnseeded)) =>
    ...
    case CommitFinished(_, block, _, Success(other)) =>
      failPoll(s"Unexpected block commit response for ${block.id}",
        new IllegalStateException(other.toString))

    case CommitFinished(_, block, _, Failure(ex)) =>
      failPoll(s"Block commit ask failed for ${block.id}", ex)

    case CursorsLoaded(_, Success(cursors), heights) =>
      Future(source.commonAncestor(cursors, heights.chain))(pollingContext)
        .map(result => AncestorLocated(incarnation, result, heights))
        .recover { case ex => AncestorLocated(incarnation, Failure(ex), heights) }
        .pipeTo(self)

    case CursorsLoaded(_, Failure(ex), _) => failPoll("Could not read retained rollback cursors", ex)

    case AncestorLocated(_, Success(Some(ancestor)), heights) =>
      (syncHandler ? RollbackTo(ancestor.blockId))
        .map(result => RollbackFinished(incarnation, Success(result), heights.usable))
        .recover { case ex => RollbackFinished(incarnation, Failure(ex), heights.usable) }
        .pipeTo(self)

    case AncestorLocated(_, Success(None), _) => beginSnapshotRecovery()
    case AncestorLocated(_, Failure(ex), _) => failPoll("Common-ancestor search failed", ex)

    case RollbackFinished(_, Success(done: RollbackCompleted), tip) =>
    ...
    case RollbackFinished(_, Success(_: RollbackRejected), _) => beginSnapshotRecovery()
    case RollbackFinished(_, Success(other), _) =>
      failPoll("Unexpected rollback response", new IllegalStateException(other.toString))
    case RollbackFinished(_, Failure(ex), _) => failPoll("Rollback ask failed", ex)
    ...
    case ResetFinished(_, Success(SyncStateReset)) =>
      cursor = None
      nextHeight = firstHeight
      ...
      seedFromMinerDictionary()
        ...
        .recover { case ex => StartupState(incarnation, Failure(ex)) }
        .pipeTo(self)
    case ResetFinished(_, Success(other)) =>
    ...
    case DictionaryPermitLoaded(_, Success(MinerDictionaryRepairPermit(at, permit))) =>
      ...
      Future(Try(new MinerDictionaryBootstrap(nodeContext.getNodeApi, protocol,
        ...
        .map(result => DictionaryRepaired(incarnation, at, permit, result))
        .recover { case ex => DictionaryRepaired(incarnation, at, permit, Failure(ex)) }
        .pipeTo(self)

    case DictionaryPermitLoaded(_, Success(rejected: BlockRejected)) =>
      logger.debug(s"Miner Dictionary repair was not started: ${rejected.reason}")
      completeDictionaryRepair()

    case DictionaryPermitLoaded(_, Success(other)) =>
      logger.warn(s"Unexpected Miner Dictionary repair-permit response: $other")
      completeDictionaryRepair()

    case DictionaryPermitLoaded(_, Failure(ex)) =>
      logger.warn(s"Could not obtain a Miner Dictionary repair permit: ${message(ex)}", ex)
      completeDictionaryRepair()

    case DictionaryRepaired(token, _, _, _) if token != incarnation => ()

    case DictionaryRepaired(_, at, permit, Success(Right(seed))) =>
      (syncHandler ? RepairMinerDictionary(at, seed.minerTree, seed.dataBoxToken, permit))
        .map(result => DictionaryRepairPublished(incarnation, Success(result)))
        .recover { case ex => DictionaryRepairPublished(incarnation, Failure(ex)) }
        .pipeTo(self)

    case DictionaryRepaired(_, _, _, Success(Left(reason))) =>
      completeDictionaryRepair()
    ...
    case DictionaryRepaired(_, _, _, Failure(ex)) =>
      completeDictionaryRepair()
      logger.warn(s"Miner Dictionary bootstrap retry could not run: ${message(ex)}", ex)

    case DictionaryRepairPublished(token, _) if token != incarnation => ()

    case DictionaryRepairPublished(_, Success(_: BlockCommitted)) =>
      completeDictionaryRepair()

    case DictionaryRepairPublished(_, Success(rejected: BlockRejected)) =>
      logger.warn(s"Rebuilt Miner Dictionary was not accepted: ${rejected.reason}")
      completeDictionaryRepair()

    case DictionaryRepairPublished(_, Success(other)) =>
      logger.warn(s"Unexpected Miner Dictionary repair response: $other")
      completeDictionaryRepair()

    case DictionaryRepairPublished(_, Failure(ex)) =>
    ...
    case QuarantinesLoaded(cycle, Success(RepairableQuarantines(faults, at, permit))) =>
      faults.headOption match {
        ...
        case Some(fault) =>
          ...
          beginRollupRepair(cycle, fault, at, permit)
      }
    ...
    case RollupRepaired(cycle, _, _, _, _) if !activeRepairCycle.contains(cycle) =>
      logger.debug(s"Ignoring result for completed repair cycle ${cycle.id}")

    case RollupRepaired(cycle, rollupId, at, permit, result) =>
      cancelRepairTimeout()
      ...
      result match {
        case Success(outcome) => publishRollupRepair(cycle, rollupId, at, permit, outcome)
        case Failure(ex) =>
          publishRollupRepair(cycle, rollupId, at, permit,
  ...
  private def publishRollupRepair(cycle: RepairCycle,
    ...
    (syncHandler ? RepairRollup(rollupId, at, outcome, permit))
  ...
  private def beginRollupRepair(cycle: RepairCycle,
    ...
    Future(new RollupRepair(nodeContext.getNodeApi, protocol,
      ...
      .map(result => RollupRepaired(cycle, fault.rollupId, at, permit, Success(result)))
      .recover { case ex => RollupRepaired(cycle, fault.rollupId, at, permit, Failure(ex)) }
      .pipeTo(self)
  }
  ...
  private def beginPoll(): Unit = {
    ...
    Future(pollCanonical())(pollingContext)
      .map(result => PollFinished(incarnation, result))
      .recover { case ex => PollFinished(incarnation, Failure(ex)) }
      .pipeTo(self)
  }
  ...
  private def commitNext(tip: Int): Unit = pending.headOption match {
    ...
    case Some(block) =>
      ...
      (syncHandler ? ApplyBlock(block))
        .map(result => CommitFinished(incarnation, block, tip, Success(result)))
        .recover { case ex => CommitFinished(incarnation, block, tip, Failure(ex)) }
        .pipeTo(self)
  }
  ...
  private def beginSnapshotRecovery(): Unit = {
    ...
    restoreSnapshot()
      .map(state => RecoveryPrepared(incarnation, Success(state)))
      .recover { case ex => RecoveryPrepared(incarnation, Failure(ex)) }
      .pipeTo(self)
  }
  ...
  private def beginFullRescan(): Unit = {
    ...
    (syncHandler ? ResetSyncState)
      .map(result => ResetFinished(incarnation, Success(result)))
      .recover { case ex => ResetFinished(incarnation, Failure(ex)) }
      .pipeTo(self)
  }
