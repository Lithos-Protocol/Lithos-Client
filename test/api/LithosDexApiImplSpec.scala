...
class LithosDexApiImplSpec
  ...
  private case class Fixture(api: transactions.engine.execution.DexAPIExecution,
                             nodeApi: NodeApi,
                             ctxOf: NodeContext,
  ...
  private def fixture(mutationWait: Long = 15000L): Fixture = {
    ...
    val impl = new transactions.engine.execution.DexAPIExecution(nodeCtx, EngineFunding(wallet.ref, 2.seconds, ec)) {
