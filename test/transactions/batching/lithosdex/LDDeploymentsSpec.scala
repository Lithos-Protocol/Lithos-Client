...
/**
 * Deployment verification against two real mainnet deployments: the canonical ERG:LIT pool, and ERG:BBC,
...
class LDDeploymentsSpec extends AnyFlatSpec with Matchers {
  ...
  private val BbcPoolNft = "77ebb7ac1a9386d6cc5b64fffa52602bea574b0bb454fb856188ae54acb84894"
  private val BbcVaultNft = "9cec4df2183f6047edf61ec9bb60649c22362e17c074fac7c41d75c5229129f0"
  private val BbcProvToken = "27bc91eaf7e72d2a17cf6e73de349262c87bea7894121963b4100a644a08da25"
  private val Bbc = "fffe6122886e3b0ab9b72b401b39bf8d3f13580c1335a41d91d19deb8038ccd4"
  // Derived by hand from the provision logic, outside this client, when the pool was first checked
  private val BbcGuardTree =
    "1b31010e205c8dfb9ff13352181a87617d8f3f29d041bb0327b4a2cd521e5e6efc61313d00ea02d193cbe4e3400e7300d40840"
  private val BbcGuardHash = "15e9919219319b00dd92fa593e5a2f7beb75e876be8154f9b1b6e9dc9415a04b"

  private val litPool = fixture("erg-lit-pool.hex")
  private val litVault = fixture("erg-lit-vault.hex")
  private val bbcPool = fixture("erg-bbc-pool.hex")
  private val bbcVault = fixture("erg-bbc-vault.hex")

  private val litAssets = Seq(LitPoolNft -> 1L, Lit -> 145230319426812L, LitProvToken -> 999999999999994L)
  private val bbcAssets = Seq(BbcPoolNft -> 1L, Bbc -> 100000000L, BbcProvToken -> 1000000000000000L)
  ...
  it should "accept a deployment someone else minted, down to its guard" in {
    val contracts = LDDeployments.verify(Mainnet, bbcPool, bbcAssets).getOrElse(fail("ERG:BBC was refused"))
    contracts.poolNFT.toString shouldBe BbcPoolNft
    contracts.vaultNFT.toString shouldBe BbcVaultNft
    contracts.feeVault.ergoTreeHex shouldBe bbcVault
    contracts.provisionGuard.ergoTreeHex shouldBe BbcGuardTree
    contracts.provisionGuard.hashedPropBytesHex shouldBe BbcGuardHash
  }
  ...
  it should "refuse a pool that names another deployment's guard" in {
    ...
    val forged = bbcPool.replace(BbcGuardHash, LitGuardHash)
    forged should not be bbcPool
    LDDeployments.verify(Mainnet, forged, bbcAssets) shouldBe None
  }

  it should "refuse a pool whose NFT is not held as a single unit" in {
    LDDeployments.verify(Mainnet, bbcPool, (BbcPoolNft -> 2L) +: bbcAssets.tail) shouldBe None
  }
  ...
  it should "leave the provision token to the vault check, which the live vault fails" in {
    ...
    val other = "11" * 32
    val contracts = LDDeployments.verify(Mainnet, bbcPool, Seq(BbcPoolNft -> 1L, Bbc -> 1L, other -> 1L))
      .getOrElse(fail("the pool tree alone does not depend on the provision token"))
    contracts.feeVault.ergoTreeHex should not be bbcVault
  }

  "DexContracts" should "hand each builder the deployment its box's own ids belong to" in {
    val bbc = LDDeployments.verify(Mainnet, bbcPool, bbcAssets).get
    DexContracts.register(bbc)

    DexContracts.forPool(Mainnet, ErgoId.create(BbcPoolNft)).liquidityPool.ergoTreeHex shouldBe bbcPool
    DexContracts.forVault(Mainnet, ErgoId.create(BbcVaultNft)).feeVault.ergoTreeHex shouldBe bbcVault
    DexContracts.forProvToken(Mainnet, ErgoId.create(BbcProvToken)).provisionGuard.ergoTreeHex shouldBe BbcGuardTree
