...
import sigma.ast.ErgoTree.{HeaderType, SizeFlag, VersionFlag, ZeroHeader}
import sigma.ast.SCollection.SByteArray
import sigma.ast.{AND, EQ, ErgoTree, FuncValue, Global, IntConstant, MethodCall, SBigInt, SGlobal, SGlobalMethods, SUnsignedBigInt, Slice, UnsignedBigIntConstant, ValUse, Value}
import sigma.{Colls, SigmaProp, VersionContext}
import sigma.data.{CSigmaProp, CUnsignedBigInt, ProveDHTuple, ProveDlog, SigmaBoolean, SigmaConjecture, SigmaLeaf, TrivialProp}
...
class CollateralSuite extends AnyFunSuite{
  ...
  test("Make fraud script"){
    localClient.execute{
      ctx =>
        val languageVersion = VersionContext.V6SoftForkVersion
        println(VersionContext.current)

          val fraudProof = FraudProofContracts.mkInvalidDiffContract(ctx)
          println(fraudProof.address(ctx.getNetworkType))
