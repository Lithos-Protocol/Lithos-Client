...
case class NonUniqueHeadersProof(contract: Contract, miner: Array[Byte], nispTree: NISPTree, evalInput: InputUTXO,
                                 fpControl: InputUTXO)
      ...
      fpAttempt.failed.get match {
        case ie: InterpreterException =>
          if(!ie.getMessage.contains("Script reduced to false")) {
            ...
            logger.info("NonUniqueHeadersProof reduced to false")
          }
        case e =>
          throw e
